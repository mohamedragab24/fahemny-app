...
### مشكلة: لا يمكن ربط المشروع بـ GitHub لوجود اتصال قديم

**الأعراض:** عند محاولة ربط المشروع بمستودع جديد على GitHub، تظهر نافذة تطلب "Remote name" كما في الصورة، أو يظهر خطأ يفيد بأن الاتصال (remote) المسمى `origin` موجود بالفعل.

**السبب:** مشروعك المحلي ما زال يحتفظ بإعدادات الاتصال القديمة التي تشير إلى المستودع المحذوف.

**الحلول:**

#### الحل السريع (من داخل الواجهة الرسومية)

عندما تظهر لك النافذة التي تطلب "Remote name":
1.  لا تكتب `origin`.
2.  بدلاً من ذلك، اكتب اسمًا جديدًا مثل `github` ثم اضغط `Enter`.
3.  هذا سيقوم بإنشاء اتصال جديد باسم مختلف ويجب أن يسمح لك بإكمال العملية بنجاح.

#### الحل الدائم (موصى به)

هذا هو الحل الأفضل لتنظيف إعدادات مشروعك بشكل كامل.

1. افتح الطرفية (Terminal) في بيئة التطوير.
2. انسخ ونفذ الأمر التالي لإزالة الاتصال القديم المسمى `origin`:
   ```bash
   git remote rm origin
   ```
3. بعد تنفيذ الأمر، حاول ربط المشروع مرة أخرى من الواجهة الرسومية. ستتمكن الآن من استخدام الاسم الافتراضي `origin` بنجاح.
